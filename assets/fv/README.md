...
- 中間部は Seedance 2.5 で2クリップ生成（日の出→燃える「誉」／燃える「誉」→金属質の金）し、
  動き補間で2.0–5.0秒に収めた。
- 2.0秒の切り替え点はディゾルブを使わずハードカット。生成クリップは頭の映像に対して横約2%・縦約0.3%の
  スケール差と、クリップ内で山が約18px縦に流れるドリフトがあったため、`assemble_stab.py` で全フレームを
  頭の最終フレームの幾何に安定化している（位相相関でスケール＋平行移動を推定、フレームごとの縦ドリフトを
  左側の山の帯で計測して打ち消し、変形で空いた画面端は頭の映像で埋める。色は一定のチャンネル別補正）。
  結果、山と湖の位置は全編で頭の映像と一致（残差0px）。
- 再構築: `python3 assemble_blaze.py sdAB3 clipA.mp4:0.08 clipB.mp4:0:3.2 0.70`（リタイム）→
  `DY_ADJ=2 python3 assemble_stab.py NAME sdAB3`（安定化・組み立て）
