...
- 中間部は Seedance 2.5 で2クリップ生成（日の出→燃える「誉」／燃える「誉」→金属質の金）し、
  動き補間で2.0–5.0秒に収めた。
- 2.0秒の切り替え点はディゾルブを使わずハードカット。クリップ側の先頭フレームを頭の最終フレームに
  色（チャンネル別ゲイン／オフセット）と位置（位相相関で推定した平行移動、黒帯が出ないよう微小ズームで補う）で
  一致させ、その補正を2秒かけて0に減衰させることで、二重像のボケを出さずに接続している。
- 再構築: `XF_IN=0 XF_OUT=0 MATCH=1 MATCH_LEN=48 python3 assemble_blaze.py NAME clipA.mp4:0.08 clipB.mp4:0:3.2 0.70`
