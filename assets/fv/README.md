...
| `fv_homare_E_predawn_keyframe_source.png` / `fv_homare_E_sunrise_keyframe_source.png` | 頭の開始（夜明け前）・終了（クリップ先頭）キーフレーム |
| `assemble_head.py` | 頭の生成クリップと中間クリップをクリップ幾何で安定化して組み立てるスクリプト（本命） |
...
| カット | 絵コンテ | 動画 |
| --- | --- | --- |
| C1 夜明け前・静止 | 0.00–1.60 | 0.0–1.88（作り直した頭。日の出は1.88秒） |
...
- 頭（0–2.0秒）は、2秒以降の生成クリップ（Seedance）の先頭フレームを基準に作り直したもの。
  先頭フレームを Nano Banana Pro で「夜明け前」に再ライティングした静止画を開始フレーム、クリップ先頭フレームを
  終了フレームとして MiniMax H3 で日の出クリップを生成（カメラ完全固定、内部ドリフト0）。日の出の瞬間が1.88秒
  （鈴のSE 1.95秒）に来るよう区分線形でリタイムし、全フレームをクリップ先頭フレームの幾何に安定化。
- 中間（2.0–5.0秒）のクリップは自身の先頭フレームの幾何にドリフト補正のみで安定化（スケール変形なし）。
  安定化で空く画面端は、そのフレーム自身の端の画素を延長して埋める（固定プレートで埋めると上端に別の空の帯が出るため）。
  結果、全編で山・湖の位置がクリップ先頭フレームと残差1px以内で一致し、2.0秒の切り替えは前後と同じ
  フレーム差（約2）で連続。ポスター（先頭フレーム）はこの版のものに更新。
- 再構築: `python3 assemble_head.py NAME head_clip.mp4 45`（45 = 日の出を置く出力フレーム）
