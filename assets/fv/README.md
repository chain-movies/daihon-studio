...
| C4 光が冷えて金属質の金に定着 | 3.30–4.60 | 4.4–5.0（最終フレームは旧本命と同一） |

- 頭（0–2.0秒）と最終フレームは旧本命の映像をそのまま使用。音声も旧本命のミックスをそのまま使用。
- 中間部は Seedance 2.5 で2クリップ生成（日の出→燃える「誉」／燃える「誉」→金属質の金）し、
  動き補間で2.0–5.0秒に収めた。切り替え点と最終フレームは10–12フレームのディゾルブで接続。
- 再構築: `python3 assemble_blaze.py NAME clipA.mp4:0.4 clipB.mp4:0:2.8 0.72`
