...
GitHub Actions だけで動く自動投稿の仕組み。サーバー不要。
**Threads は無料、X は従量課金**（1日3本で月200〜850円程度 → 「制限とコスト」）。
...
1. https://developer.x.com/ で開発者アカウントを作る
   - 登録時に「開発者PPUパイロット契約」への同意を求められる。**これは従量課金の契約**で、
     2026年2月6日以降 X API に無料枠はない（→「制限とコスト」）
   - ユースケース欄には、自社の告知を自動投稿する用途であることを具体的に書く
   - 続けてアプリ（App）を作る
...
### X — 無料枠はない（2026年2月6日に廃止）

X API は**従量課金（Pay-Per-Use）のみ**。開発者コンソールでクレジットを前払いで購入し、リクエストごとに引かれる。

| 操作 | 単価 |
|---|---|
| 投稿の作成 | $0.015 |
| **URL を含む投稿の作成** | **$0.200**（13倍） |
| 投稿の読み取り | $0.005 |

この設定（1日3本 = 月約90本）での概算：

| 内容 | 月額 |
|---|---|
| URL なし | 約 **$1.35**（200円前後） |
| 4本に1回 CTA の URL を入れる | 約 **$5.61**（850円前後） |

URL 1個で単価が13倍になるため、`config.json` の `cta.urlPlatforms` は既定で `["threads"]`
（無料の Threads にだけリンクを出し、X はプロフィール誘導にする）。
X にもリンクを出したい場合だけ `["x", "threads"]` に変える。

`post.mjs` は X へ送る前に概算コストをログに出すので、Actions の実行ログで実費が追える。
開発者コンソールで**上限額（spend cap）と自動チャージのしきい値**を必ず設定しておくこと。

> 単価も課金体系も2026年に入って複数回変わっている。クレジット購入前に
> [開発者コンソールの料金表](https://docs.x.com/x-api/getting-started/pricing) で最新の数字を確認すること。

### Threads — 無料

1日250件まで。この設定では1日3本なので余裕がある。

### GitHub Actions

public リポジトリなら無料。private でも1回数秒なので無料枠にほぼ影響しない。
...
| X が 429 | レート上限に到達。スロット数を減らす |
| X が 402 / クレジット不足のエラー | 開発者コンソールでクレジットを追加購入する（従量課金） |
